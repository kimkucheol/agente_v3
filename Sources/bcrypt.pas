...
const
	...
	BCRYPT_COST = 8; //cost determintes the number of rounds. 10 = 2^10 rounds (1024)
	BCRYPT_SALT_LEN = 12; //bcrypt uses 128-bit (16-byte) salt (This isn't an adjustable parameter, just a name for a constant)
...
begin
	...
	strLen := WideCharToMultiByte(CodePage, 0,
			PWideChar(Source), Length(Source), //Source
{aqui como Ansi}			PAnsiChar(cpStr), strLen, //Destination
			nil, nil);
	if strLen = 0 then
	begin
		dw := GetLastError;
